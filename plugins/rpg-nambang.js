let handler = async (m, { conn }) => {
  ...
  if (new Date - global.db.data.users[m.sender].lastnambang > 300000) {
      ...
      let arr = [
          ...
          🪨🪨🪨🪨🪨🪨🪨⛏️🪨🪨
          🪨🪨⛏️⛏️🪙  🪙 🪨       \n\n\n➕ du in minen...`, 
          `➕ 💹erhalten Ergebnis minen....`, 
          `*—[ Ergebnis nambang du ${name} ]—*
          ➕ 🪨 coal = [ ${zero5} ]
          ➕ ✨ Gold = [ ${zero4} ] 	
          ➕ ✨ Diamant = [ ${zero3} ] 		 
          ${wm}`
      ...
      let { key } = await conn.sendMessage(m.chat, {text: 'suchen Ort nambang.....'})
  ...
  } else m.reply(`Sepertinya Sie bereits kecapekan von minen... bitte Pause früher ungefähr\n*${timers}*`)
}
