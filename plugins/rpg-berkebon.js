const timeout = 1800000
                                     ...
                                     if (apelu == 0 || angguru == 0 || manggau == 0 || pisangu == 0 || jeruku == 0) return conn.reply(m.chat, `*Pastikan du haben alle bibit*\n*Wie Bibit Apel, Bibit Mangga, Bibit Jeruk, Bibit Pisang, Bibit Anggur*\n\nTippe :\n${usedPrefix}shop buy bibitmangga 500\n\n*List*\nbibitmangga\nbibitanggur\nbibitpisang\nbibitjeruk\nbibitapel`, m)
                                     if (new Date - global.db.data.users[m.sender].lastberkebon< 1800000) return conn.reply(m.chat, `Sie bereits menanam\nBitte warten Ergebnis panenmu\nTunggu während ${msToTime(time - new Date())} wieder`, m)
                                     ...
                                     setTimeout(() => {
					                      conn.reply(m.chat, `Waktunya berkebon wieder kak 😅`, m)
					                  }, timeout)
                                  } else m.reply(`Pastikan bibit anggur du *500* für kann berkebon`)
                              } else conn.reply(m.chat, `Pastikan bibit jeruk du *500* für kann berkebon`, m)
                          } else conn.reply(m.chat, `Pastikan bibit pisang du *500* für kann berkebon`, m)
                      } else conn.reply(m.chat, `Pastikan bibit apel du *500* für kann berkebon`, m)
                  } else conn.reply(m.chat, `Pastikan bibit mangga du *500* für kann berkebon`, m)
              }
