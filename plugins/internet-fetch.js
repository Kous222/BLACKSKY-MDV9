//darf dicopy kak hehe:v
// wm © BOTCAHX nicht in ilangin

const axios = require('axios');
let handler = async (m, { text, args }) => {
  if (!/^https?:\/\//.test(text)) throw 'Anfangi *url* mit http:// oder https://';
  try {
    ...
    if (chSize.oversize) return conn.reply(m.chat, ` 🚩 File size (${size}) exceeds the maximum limit, we can't herunterladen the file.`, m);
