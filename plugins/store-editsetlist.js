...
const handler = async (message, { text, isOwner, usedPrefix }) => {
    ...
    if (!isOwner) throw `Nur der Eigentümer kann die Setlist bearbeiten.`;
    if (!text) throw `Bitte gib die Setlist an, die konfiguriert werden soll. Beispiel: ${usedPrefix}bearbeitensetlist SetlistText\n\nAnweisung: Verwende benutzerdefinierten Text für das Layout der Setlist. Beispielvorlage:\n🔥 BetaBotz Hosting 🔥  
...
Gib das Schlüsselwort ein, um den Inhalt zu sehen!`;
    ...
    return message.reply(`Setlist für diese Gruppe erfolgreich bearbeitet!`);
};

handler.help = ['bearbeitensetlist'];
handler.tags = ['store'];
handler.command = /^bearbeitensetlist$/i;
handler.owner = true;
