function listAfkHandler(m, { conn }) {
    let users = Object.entries(global.db.data.users)
        ...
        .map(([jid, user]) => {
            ...
            let reason = user.afkReason ? '• *ALASAN* : ' + user.afkReason : '• *OHNE ALASAN*'
            let duration = new Date() - user.afk
            return `${name} (Jangka Zeit: ${formatTime(duration)})\n${reason}`
        })
    ...
    if (users.length > 0) {
        ...
        conn.reply(m.chat, 'Nein gibt Nutzer das/der/die gerade AFK wenn dies.', m)
    }
}
...
function formatTime(ms) {
    ...
    let timeString = ''
    if (days > 0) timeString += `${days} Tag `
    if (hours > 0) timeString += `${hours} jam `
    if (minutes > 0) timeString += `${minutes} menit `
    if (seconds > 0) timeString += `${seconds} Sekunden`
