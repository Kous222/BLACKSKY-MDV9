...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    try {
        ...
        let res = await fetch(`https://api.betabotz.eu.org/api/primbon/harinaas?tanggal=${tanggal1}&Monat=${bulan1}&Jahr=${tahun1}&apikey=${lann}`);
        let json = await res.json();
        let anu = [
          `―-HARI NAAS-―\n\nTage lahir: ${json.result.message.hari_lahir}\n\nTanggal lahir: ${json.result.message.tgl_lahir}\n\nTage naas: ${json.result.message.hari_naas}\n\nCatatan: ${json.result.message.catatan}\n\nInfo: ${json.result.message.info}`, 
       ]
        if (json.Status) {
