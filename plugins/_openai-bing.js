...
let handler = async (m, {
  ...
  if (command == 'bing') {
    if (!text) throw `Example : ${usedPrefix + command} wer presiden Indonesia?`;
    try {
      m.reply(wait)
  ...
  if (command == 'bingimg') {
    if (!text) throw `Contoh: ${usedPrefix + command} anak rennen benutzen pakaian merah 3d animation`;
    try {
      m.reply(wait)
