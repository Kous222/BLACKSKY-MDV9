...
exports.makeWASocket = (connectionOptions, options = {}) => {
    ...
    // Contacts and Groups update names
    conn.ev.on('contacts.upsert', updateNameToDb)
    conn.ev.on('groups.update', updateNameToDb)

    // Handle chat set events
    ...
    for (let { id, name, readOnly } of chats) {
        ...
        let chat = conn.chats[jid] ||= { id: jid }
    ...
    for (const update of updates) {
        const jid = conn.decodeJid(update.id)
    ...
    // Chats upsert (e.g. new chats created)
    conn.ev.on('chats.upsert', async ({ id, name }) => {
    const jid = conn.decodeJid(id)
    if (!jid) return
    ...
    const senderId = Object.keys(presences)[0] || id
    const sender = conn.decodeJid(senderId)
    const presence = presences[senderId]?.lastKnownPresence || 'composing'

    let senderChat = conn.chats[sender] ||= { id: sender }
   ...
   conn.getFile = async (PATH, returnAsFilename) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        const on = (...args) => {
            tries++;
            if (tries > maxTries) {
                conn.ev.off(eventName, on);  // Listener entfernen
