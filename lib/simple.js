...
exports.makeWASocket = (connectionOptions, options = {}) => {
  ...
  conn.ev.on('chats.upsert', async function chatsUpsertPushToDb(chatsUpsert) {
    ...
    if (id.endsWith('@g.us')) {
      ...
      for (const group in groups) {
        conn.chats[group] = { id: group, subject: groups[group].subject, isChats: true, metadata: groups[group] };
      }
    }
  });
  ...
  conn.filter = (text) => {
    ...
    let res = Array.from(text).filter(v => vowels.includes(v));
    let resu = res[res.length - 1];
    for (let huruf of vowels) {
  ...
  conn.resize = async (buffer, uk1, uk2) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const resizedImage = await image.resize(uk1, uk2).getBufferAsync(jimp.MIME_JPEG);
  ...
  conn.sendMedia = async (jid, path, quoted, options = {}) => {
    const { ext, mime, data } = await conn.getFile(path);
  ...
  conn.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
    ...
    const { res, data: file, filename: pathFile } = type;
    ...
    if (/webp/.test(type.mime)) mtype = 'sticker';
      ...
      const convert = await (ptt ? toPTT : toAudio)(file, type.ext);
      file = convert.data;
...
`.trim();
  ...
  conn.sendButton = async (jid, contentText, footer, buffer, buttons, quoted, options) => {
    ...
    let message = {
      ...
      footer,
      buttons: buttons.map(btn => ({ buttonId: btn[1] || btn[0] || '', buttonText: { displayText: btn[0] || btn[1] || '' } })),
      ...(buffer ? { image: buffer } : {})
    };
  ...
  conn.sendButtonImg = async (jid, buffer, contentText, footerText, button1, id1, quoted, options) => {
    ...
    if (res && res.status !== 200 || file.length <= 6553
